...
typedef boost::shared_ptr<model::Event> event_ptr;
...
public:
	...
	/* event */
	int ev_add(model::Event& event);
	int ev_update(model::Event& event);
	int ev_get(string id, model::Event& event);
	int ev_del(string id);

	/* activity */
	int act_put(string entity, uint64_t type, string event);
	bool act_exist(string entity, uint64_t type, string event);
	int act_del(string entity, uint64_t type, string event);
	act_iter_ptr act_first(string entity, uint64_t type);
	act_iter_ptr act_last(string entity, uint64_t type);
	act_iter_ptr act_seek(string entity, uint64_t type, string from);
...
private:
	...
	Database* db;
public:
	act_iterator(string eid, uint64_t type, leveldb::Iterator* it, Database* db)
	{
		this->eid = eid;
		this->type = type;
		this->it = it;
...
public:
	...
	void ev_add(model::Event& event);
	void ev_update(model::Event& event);
	void ev_del(string id);
	...
	void act_put(string e1, uint64_t type, string e2);
	void act_del(string e1, uint64_t type, string e2);
