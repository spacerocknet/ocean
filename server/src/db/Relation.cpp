...
relation_ptr rel_iterator::next3()
{
	uint32_t meta;
	string source;
	uint64_t type;
	...
	if (!it->Valid()) return relation_ptr();
	_dec_declare2_(k, it->key().data(), it->key().size());
	_dec_get_var32_(k, meta);
	_dec_get_string_(k, source);
	_dec_get_var64_(k, type);
	...
	if (!_dec_valid_(k)) return relation_ptr();
	if ((meta != model::RELATION) || (source != eid) || (type != this->type)) return relation_ptr();
	relation_ptr ret = boost::make_shared<model::Relation>();
	if (!ret->ParseFromArray(it->value().data(), it->value().size())) return relation_ptr();
	it->Next();
	return ret;
}
